...
export const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  return (
    <>
      ...
      <section className="grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mt-2">
        {addedPhotos?.length > 0 &&
          addedPhotos.map((link) => (
            <div key={link} className="h-32 flex relative">
              <img
                src={"http://localhost:40000/uploads/" + link}
                className="rounded-2xl w-full object-cover"
              />

              <button
                className="absolute bottom-1 right-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3"
              ...
              <button
                className="absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl py-2 px-3"
