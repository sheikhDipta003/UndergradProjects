$(document).ready(function() {    
    ...
    $(document).on("change", "#multiple_files", function(e) {
        ...
        $.each(files, function(i, file) {
            ...
            reader.onload = function(e) {
                ...
                var template = '<li class="ui-state-default del" style="position:relative;"><img id="' + file.name + '" style="width:60px; height:60px" src="' + e.target.result + '"></li>';
        ...
        if (files.length != 0) {
            if (files.length > 10) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    storeImage += '{\"imageName\":\"user/' + userid + '/postImage/' + name + '\"},';
    ...
    $(document).on('click', '.postImage', function() {
        ...
        console.log('userid = ', userid, ', postid = ', postid, ', imageSrc = ', imageSrc);

        $.post('http://localhost/facebook/core/ajax/imgFetchShow/imgFetchShow.php', {
            fetchImgInfo: userid,
            postid: postid,
            imageSrc: imageSrc
        }).done(function(data) {
    ...
    $(document).mouseup(function(e) {
        var containers = [
            { element: $('.add-cov-opt'), action: function() { $(this.element).hide(); } },
            { element: $('.profile-dialoge-show'), action: function() { $(this.element).hide(); } },
            { element: $('.profile-status-write'), action: function() { $('.status-share-button-wrap').hide('0.2'); } },
            { element: $('.post-img-wrap'), action: function() { $('.top-wrap').remove(); } },
            { element: $('.post-option-details-container'), action: function() { $(this.element).empty(); } },
            { element: $('.top-box-show'), action: function() { $(this.element).empty(); } },
        ];
    
        $.each(containers, function(index, container) {
            if (!container.element.is(e.target) && container.element.has(e.target).length === 0) {
                container.action();
            }
        });
    });
