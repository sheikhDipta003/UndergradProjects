    ...
    $(document).on('click', '.post-option', function() {
        ...
        $(postDetails).show().html(
            '<div class="post-option-details">' +
                '<ul>' +
                    '<li class="post-edit" data-postid="' + postid + '" data-userid="' + userid + '">Edit</li>' +
                    '<li class="post-delete" data-postid="' + postid + '" data-userid="' + userid + '">Delete</li>' +
                    '<li class="post-privacy" data-postid="' + postid + '" data-userid="' + userid + '">Privacy</li>' +
                '</ul>' +
            '</div>'
        );
    })
        ...
        $('.top-box-show').html(
            '<section class="top-box profile-dialog-show" style="top: 12.5%;left: 22.5%;width: 55%;">' +
                ...
                '<section class="edit-post-submit" style="position: absolute;right:0; bottom: 0; display: flex; align-items: center; margin: 10px;">' +
                    ...
                    '</div>' +
                    '<div class="edit-post-save" style="padding: 3px 15px; background-color: #4267bc;color: white; font-size: 14px; margin-left:5px; cursor:pointer;" data-postid="' + postid + '" data-userid="' + userid + '" data-tag="' + getPostImg + '">Save</div>' +
                '</section>' +
            '</section>'
        );
    })
